...
def test(data,
    ...
    # Dataloader
    print("debug # test float 3 ",device.type, device)
    if device.type != 'cpu':
        model(torch.zeros(1, 3, imgsz, imgsz).to(device).type_as(next(model.parameters())))  # run once
        print("debug test_float model.model[0].conv.bias :",model.model[0].conv.bias)
        print("debug test_float model.model[0].conv.weight :",model.model[0].conv.weight)
        print("debug test_float model[105].m[0].bias :",model.model[105].m[0].bias)
        print("debug test_float model[105].m[0].weight :",model.model[105].m[0].weight)
    ...
    for batch_i, (img, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        ...
        with torch.no_grad():
            ...
            # print("before call model ")
            xx = model(img)  # inference and training outputs
            # print("debug purposes #0 ____________xx shape: " ,"  ", xx[0].shape,"  ", xx[1].shape,"  ", xx[2].shape)
            ...
            for i in range(nl):
                debug = True
                if not debug:
                    xx[i] = xx[i].permute(0,1,3,4,2).contiguous() # to conter permute from quantization
                    bs, _, ny, nx, _no = xx[i].shape
                else:
                    bs,na,ny,nx,no = xx[i].shape
                    
                # print("debug # postprocess 1 bs, na, ny, nx, no",bs,"  ",na,"  ",ny,"  ",nx,"  ",no)

                if grid[i].shape[2:4] != xx[i].shape[2:4]:
                    # print("debug # 2 test_float grid shape ", i , "  ",grid[i].shape, "  ", xx[i].shape )
                    grid[i] = _make_grid(nx, ny).to(xx[i].device)
                    # print("debug # 3 test_float grid shape ", i , "  ",grid[i].shape, "  ", xx[i].shape )
                y = xx[i].sigmoid() # (tensor): (b, self.na, h, w, self.no)
            ...
            out,train_out = torch.cat(z,1),xx
            ...
            if compute_loss:
                ...
                print("debug test_float #1 ",compute_loss)

            # Run NMS
            targets[:, 2:] *= torch.Tensor([width, height, width, height]).to(device)  # to pixels
            lb = [targets[targets[:, 0] == i, 1:] for i in range(nb)] if save_hybrid else []  # for autolabelling
            t = time_synchronized()
            # print("debug # postprocess 4 targets.shape:", targets.shape, " targets: ", targets)
            out = non_max_suppression(out, conf_thres=conf_thres, iou_thres=iou_thres, labels=lb, multi_label=True)
            # for i in range(len(out)):
            #     print("debug # postprocess 3 i: ", i, "  " , out, " out shape: ", len(out[0]))
    ...
    # Save JSON
    print("debug test_float # 3 save_json ", save_json, len(jdict), '\n',jdict)
    if save_json and len(jdict):
        w = Path(weights[0] if isinstance(weights, list) else weights).stem if weights is not None else ''  # weights
        anno_json = '/data/coco_quant/coco/annotations/instances_val2017.json'  # annotations json
